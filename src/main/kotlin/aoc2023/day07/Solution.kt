...
fun List<String>.toCamelCards(): List<Pair<String, Int>> =
    this.map {
        val (hand, bid) = it.split(" ")
        return@map hand to bid.toInt()
    }

fun Char.cardValue(deck: String): Int = deck.withIndex().first { (_, card) -> this == card }.index
...
fun main() {
    val camelCards = "aoc2023/Day07Input.txt".filePathToStringList().toCamelCards()
